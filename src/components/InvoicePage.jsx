...
export default function InvoicePage(){
 ...
 const period=useMemo(()=>getPeriodForDate(anchor,new Date()),[anchor]);
 const summary=useMemo(()=>summarizePeriod(logs,period),[logs,period]);
 const prefill={periodId:period.id,startDate:period.start,endDate:period.end,amount:summary.expected,stops:summary.stops};
 ...
 return <motion.div className="mx-auto max-w-2xl space-y-5 pb-24 pt-2" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}>
  <header><h1 className="text-3xl font-bold">Invoice</h1><p className="mt-1 text-sm text-[#8e9ab2]">Turn your work records into an invoice.</p></header>
  <section className="rounded-[24px] border border-[#26314a] bg-[#111827] p-5 shadow-xl shadow-black/10">
   <div className="flex items-start gap-3"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#7567ff]/15 text-[#9b91ff]"><CalendarDays/></span><div className="min-w-0 flex-1"><div className="text-xs text-[#8e9ab2]">Current Pay Period</div><div className="mt-0.5 text-lg font-bold">{fmt(period.start)} – {fmt(period.end)}</div><div className="text-xs text-[#6f7b92]">4 weeks</div></div><span className="rounded-full bg-emerald-500/10 px-3 py-1 text-[11px] font-bold text-emerald-400">{new Date()>new Date(period.end+"T23:59:59")?"Ready to invoice":"Current period"}</span></div>
   ...
   <p className="mt-4 rounded-2xl bg-[#172033] p-3 text-xs leading-5 text-[#a3aec1]">All totals come from your logged work. Confirm the stops once before generating, then review the invoice before sharing.</p>
