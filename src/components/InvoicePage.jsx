...
export default function InvoicePage(){
 ...
 const period=useMemo(()=>{const previous=periods[1];const alreadyInvoiced=previous&&invoices.some(i=>i.periodId===previous.id);return previous&&!alreadyInvoiced?previous:currentPeriod;},[periods,invoices,currentPeriod]);
 ...
 return <motion.div className="mx-auto max-w-2xl space-y-5 pb-24 pt-2" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}>
  ...
  <section className="rounded-[24px] border border-[#26314a] bg-[#111827] p-5 shadow-xl shadow-black/10">
   <div className="flex items-start gap-3"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#7567ff]/15 text-[#9b91ff]"><CalendarDays/></span><div className="min-w-0 flex-1"><div className="text-xs text-[#8e9ab2]">{isCurrent?"Current Pay Period":"Completed Pay Period"}</div><div className="mt-0.5 text-lg font-bold">{fmt(period.start)} – {fmt(period.end)}</div><div className="text-xs text-[#6f7b92]">4 weeks</div></div><span className="rounded-full bg-emerald-500/10 px-3 py-1 text-[11px] font-bold text-emerald-400">{isCurrent?"Current period":"Ready to invoice"}</span></div>
