...
export default function InvoicePage(){
 ...
 return <motion.div className="mx-auto max-w-2xl space-y-4 pb-24 -mt-2" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}}>
  <header className="flex items-center justify-between gap-3"><h1 className="text-2xl font-bold">Invoice</h1><span className="text-xs text-[#8e9ab2]">Check · create · send</span></header>
  <section className="rounded-[24px] border border-[#26314a] bg-[#111827] p-5 shadow-xl shadow-black/10">
   ...
   <button data-tour="invoice-generate" onClick={()=>setView("create")} className="mt-4 flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#6657f5] to-[#806cff] font-bold text-white shadow-lg shadow-[#7567ff]/20"><FileText className="h-5 w-5"/> Generate Invoice <ChevronRight className="h-5 w-5"/></button>
  </section>
